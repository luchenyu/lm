...
def sentence_to_token_ids(text, vocab):
  ...
  seq = [idx if idx else vocab.key2idx("_UNK") for idx in seq]
  return seq

def token_ids_to_sentence(token_ids, vocab):
  """decode a sequence of token ids to a sentence
  """
  token_ids = filter(lambda i: i != vocab.key2idx("_PAD"), token_ids)
  token_ids = map(lambda i: i if vocab.idx2key(i) else vocab.key2idx("_UNK"), token_ids)
